...
def get_caipu_diffcul_time(cooing_part):
    """
    获取菜谱的烹饪难度
    :param cooing_part:
    :return:
...
def analysize_cooking_step(cooking_step, stepdic, author_nickname, caipu_name):
    """
    获取步骤和图片
    :param cooking_step:
...
if __name__ == '__main__':
    ...
    if dbh.caipuku.find({"caipu_url": target_url}).count() != 0:
        print "该菜谱对应的链接已经入库"
    else:
        print "该菜谱对应的链接没有入库"
